...
static void date_update_proc(Layer *layer, GContext *ctx) {
  ...
  // create the char array of the name
  name_buffer[0] = 'Y';
  name_buffer[1] = 'o';
  name_buffer[2] = 'n';
  name_buffer[3] = 'g';
  ...
  text_layer_set_text(name_label, name_buffer);
...
static void window_load(Window *window) {
  ...
  // init name
  name_label = text_layer_create(GRect(42, 25, 65, 31));
  text_layer_set_text(name_label, name_buffer);
  text_layer_set_background_color(name_label, GColorBlack);
  text_layer_set_text_color(name_label, GColorWhite);
  GFont norm28 = fonts_get_system_font(FONT_KEY_GOTHIC_28);
  text_layer_set_font(name_label, norm28);

  layer_add_child(date_layer, text_layer_get_layer(name_label));
